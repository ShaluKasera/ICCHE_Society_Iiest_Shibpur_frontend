...
const Volunteers = () => {
  ...
  const [showDetails, setShowDetails] = useState(null); // Store the id of the card with open details

  const containerRef = useRef(); // To detect clicks outside the details

  const toggleDetails = (id) => {
    setShowDetails((prev) => (prev === id ? null : id)); // Toggle details visibility
  };
  ...
  useEffect(() => {
    ...
    // Event listener for clicks outside to close details
    const handleClickOutside = (event) => {
      if (containerRef.current && !containerRef.current.contains(event.target)) {
        setShowDetails(null); // Close details when clicking outside
      }
    };
    ...
    <Layout>
      <div className="container mx-auto px-1">
        ...
        <div
          ...
          ref={containerRef} // Reference the container to detect clicks outside
        >
