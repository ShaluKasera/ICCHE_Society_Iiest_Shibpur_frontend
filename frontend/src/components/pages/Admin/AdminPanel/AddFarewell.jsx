...
const AddFarewell = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "https://iccheweb.vercel.app/api/admin/dashboard/events/farewells/add-farewell",
        formDataToSend,
        {
          headers: {
    ...
    <Layout>
      ...
      <div className="w-[90%] max-w-xl mx-auto p-6  bg-white rounded-lg shadow-lg mt-7 text-xs sm:text-base mb-7 ">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
          ...
          <input
            type="number"
            name="finalYearStudentsPresent"
            placeholder="Final Year Students Present"
            value={formData.finalYearStudentsPresent}
            onChange={handleChange}
            required
            className="border p-2 rounded focus:ring-2 focus:ring-gray-400 focus:outline-none transition"
          />
          ...
          <input
            type="number"
            name="juniorPresent"
            placeholder="Juniors Present"
            value={formData.juniorPresent}
            onChange={handleChange}
            required
            className="border p-2 rounded focus:ring-2 focus:ring-gray-400 focus:outline-none transition"
          />
